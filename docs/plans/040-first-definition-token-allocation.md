...
性能ゲートへフェーズ別計測と、変更前が失敗するローカル400 msの初回Definition上限を追加する。
...
変更前が失敗するローカル400 ms上限と、入力4倍時の正規化増加率を併用する。

GitHub Ubuntu runnerでは、約1.58 MB初回snapshotがローカル84.1 msに対して230.5 ms、約6.3 MB初回Definitionが349.0 msに対して970.7 msだった。
CIの絶対上限は1,100 msへ校正し、ローカル上限400 msと分ける。
入力4倍時の正規化増加率は1.1で、線形契約を維持している。
