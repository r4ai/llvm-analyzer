...
- capability ↔ analyzer クエリの対応:
  - `hover` ← `symbolAt` + 型/ドキュメント辞書。シンボル参照では英語の短い markdown を返し、宣言形のコードブロック、Kind / Type / Scope の表、定義元行（parameter は関数シグネチャ、local は定義命令）を表示する。opcode/type token では短い LangRef 補助説明を返す
...
- CFG 表示など VSCode 固有の UI は extension 側の command として実装し、グラフ構築自体は analyzer の純粋モデルに置く。初期表示形式は Mermaid を markdown の untitled document として開き、Webview の作り込みは後続に回す。
