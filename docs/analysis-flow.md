...
`IncrementalParserSession.create(source)`は初回のソースと`ParseResult`を保持します。
`session.update(source, edit)`は更新前の範囲と更新後の挿入終端を受け取り、トップレベル要素を二分探索します。
編集が一つの要素の内側へ厳密に収まる場合だけ、その要素を再パースします。
要素間の編集、関数境界の破壊、全文変更では`parseModule`による全文解析へ戻ります。

ソース長を`n`、トップレベル要素数を`m`、再パースする要素長を`k`、位置が移動する後続ASTノード数を`a_s`とします。
初回解析は`O(n)`です。
同じ長さの局所編集は`O(log m + k + m)`、後続位置が動く編集は`O(log m + k + m + a_s)`です。
公開ASTが絶対位置を持つ配列であるため、局所編集にも配列再構成の`O(m)`が残ります。
...
| 6    | モデル作成             | 参照列、定義範囲、可視スコープ、関数範囲の索引を持つ不変の`SemanticModel`として返す。 |

関数本体では関数スコープを優先します。
ただし型位置の `%T` は、同名のローカル値よりモジュールスコープの名前付き型を優先します。
ラベル参照は `%exit` から `exit` へ正規化して関数スコープを引きます。

`SemanticModel.occurrenceAt`と`controlFlowGraphAt`はソース順索引を二分探索します。
`symbolsInRange`は定義終端の境界だけを二分探索し、範囲内のシンボルだけを返します。
`visibleSymbolsAt`はモジュールスコープと現在関数のスコープを索引から結合します。
`referencesOf`と`occurrences`は意味解析時に整列した配列を返し、問い合わせ時に再整列しません。
